...
let UIManager = cc.Class({
    ...
    checkPopupSeq () {
        if (this.arrPopupDialog.length > 0) {
            ...
            if (!first.isShow) {
                this.showSharedDialog(first.panelPath, first.scriptName, first.param);
    ...
    _showTipsAni (content, cb) {
        ...
        resourceUtil.getUIPrefabRes('common/tips', function (err, prefab) {
            ...
            txtLabel.maxWidth = 0;
            txtLabel.string = '<color=#001D34>'+ content +'</color>';
    ...
    showMessageBox (title, content, confirmCallback, cancelCallback) {
        this.showSharedDialog('common/messageBox', 'messageBox', [title, content, confirmCallback, cancelCallback]);
    }
